...
- **Pull requests en `ws status`** - El estado de un workspace no decía si sus ramas ya tenían pull request ni en qué situación estaba: había que ir al servidor repo a repo
  ...
  - Indicador de la construcción más reciente del commit del pull request: ✅ correcta, ❌ fallida, ⏳ en curso. Sale del estado de build que publica el servidor de integración continua en Bitbucket. Manda la más reciente porque el servidor registra las construcciones por commit, no por pull request: un job anterior de otro pull request sobre el mismo commit no debe teñir el indicador. Sin construcciones, o si la más reciente se canceló, no aparece nada; si ese dato falla, el pull request se muestra igual
  - Una construcción en curso desde hace más de `WS_BITBUCKET_BUILD_STALE_HOURS` horas (6 por defecto) se muestra como `⚠️ sin terminar`. El servidor conserva el «en curso» de un build que desapareció sin publicar su final, por ejemplo al borrar su rama tras el merge, y ese estado no cambia nunca
