...
- **Pull requests en `ws status`** - El estado de un workspace no decía si sus ramas ya tenían pull request ni en qué situación estaba: había que ir al servidor repo a repo
  ...
  - Indicador de la construcción del commit del pull request: ✅ correcta, ❌ fallida, ⏳ en curso. Sale del estado de build que publica el servidor de integración continua en Bitbucket; sin construcciones no aparece nada y, si ese dato falla, el pull request se muestra igual
